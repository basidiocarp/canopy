...
fn add_task_actions(
    ...
    {
        let (kind, label, summary) = match (deadline_kind, deadline_summary.active_deadline_state) {
            ...
            (TaskDeadlineKind::Execution, DeadlineState::Overdue) => (
                ...
                "Execution deadline has passed and needs operator follow-through.".to_string(),
            ),
    ...
    if task.status == TaskStatus::Assigned && task.owner_agent_id.is_some() {
        let has_prior_execution = execution_summary.is_some_and(|summary| summary.run_count > 0);
        actions.push(OperatorAction {
            action_id: format!(
                "task:{}:{}",
                task.task_id,
        ...
        .is_some_and(|summary| summary.blocker_count > 0 || summary.blocking_count > 0)
    {
        actions.push(OperatorAction {
            ...
            summary: "Task still has dependency edges that may need to be removed or rewritten."
                .to_string(),
            due_at: None,
            expires_at: None,
        });
        ...
        && relationship_summary.is_some_and(|summary| summary.blocker_count == 0)
    {
        actions.push(OperatorAction {
            ...
            summary: "Task is blocked without remaining dependency blockers and can be reopened."
                .to_string(),
            due_at: None,
            expires_at: None,
        });
...
fn add_allowed_handoff_actions_for_item(
    ...
    actions.push(OperatorAction {
        ...
        summary: "Mark the open handoff as completed once the requested work lands.".to_string(),
        due_at: handoff.due_at.clone(),
        expires_at: handoff.expires_at.clone(),
    });
    actions.push(OperatorAction {
