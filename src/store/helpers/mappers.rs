...
pub(crate) fn map_task(row: &rusqlite::Row<'_>) -> rusqlite::Result<Task> {
    Ok(Task {
        ...
        queue_state_id: row.get(7)?,
        worktree_binding_id: row.get(8)?,
        execution_session_ref: row.get(9)?,
        review_cycle_id: row.get(10)?,
        workflow_id: row.get(11)?,
        phase_id: row.get(12)?,
        required_role: parse_optional_enum_column(row, 13)?,
        required_capabilities: row
            .get::<_, Option<String>>(14)?
            .map_or_else(Vec::new, |json| parse_capabilities(&json)),
        auto_review: row.get::<_, Option<i64>>(15)?.unwrap_or(0) != 0,
        verification_required: row.get::<_, Option<i64>>(16)?.unwrap_or(0) != 0,
        status: parse_enum_column(row, 17)?,
        verification_state: parse_enum_column(row, 18)?,
        priority: parse_enum_column(row, 19)?,
        severity: parse_enum_column(row, 20)?,
        owner_agent_id: row.get(21)?,
        owner_note: row.get(22)?,
        acknowledged_by: row.get(23)?,
        acknowledged_at: row.get(24)?,
        blocked_reason: row.get(25)?,
        verified_by: row.get(26)?,
        verified_at: row.get(27)?,
        closed_by: row.get(28)?,
        closure_summary: row.get(29)?,
        closed_at: row.get(30)?,
        due_at: row.get(31)?,
        review_due_at: row.get(32)?,
        scope: row
