...
impl Store {
    ...
    pub(crate) fn apply_task_graph_action(
        ...
        let task = match action {
            OperatorActionKind::ResolveDependency => {
                self.apply_resolve_dependency_action(task_id, changed_by, input)?
            }
            OperatorActionKind::PromoteFollowUp => {
                self.apply_promote_follow_up_action(task_id, changed_by, input)?
            }
            OperatorActionKind::CloseFollowUpChain => {
                self.apply_close_follow_up_chain_action(task_id, changed_by)?
            }
            _ => return Ok(None),
        };
    ...
    fn apply_resolve_dependency_action(
        &self,
        task_id: &str,
        changed_by: &str,
        input: &super::TaskOperatorActionInput<'_>,
    ...
    fn apply_promote_follow_up_action(
        &self,
        task_id: &str,
        changed_by: &str,
        input: &super::TaskOperatorActionInput<'_>,
    ...
    fn apply_close_follow_up_chain_action(
        &self,
        task_id: &str,
        changed_by: &str,
    ) -> StoreResult<Task> {
...
fn record_relationship_event(
    ...
    let note = format!(
        "relationship_id={}; action={action}; kind={}; role={}; related_task_id={}; related_title={}",
