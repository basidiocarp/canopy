...
fn migrations() -> Migrations<'static> {
    Migrations::new(vec![
        ...
        M::up_with_hook("", |tx| {
            let _ = tx.execute(
                "ALTER TABLE tasks ADD COLUMN completion_signal TEXT NULL",
                [],
            );
            Ok(())
        }),
...
fn add_handoffs_columns(conn: &Connection) -> rusqlite::Result<()> {
    ...
    let _ = conn.execute(
        "ALTER TABLE handoffs ADD COLUMN assignee_type TEXT NOT NULL DEFAULT 'unassigned'",
        [],
    );
